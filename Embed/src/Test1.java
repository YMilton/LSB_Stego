...
public class Test1 {
	...
	public static void main(String[] args) throws Exception{
		...
		byte[] arr = readfile.toByteArray("F:/a.pdf");
		...
		/*彩色图片整合的一维向量的获得*/
		ArrayList<int[]> rgblist = getImageRGBList(getImageRGB("F:/img1.jpg"));
	
		int[] rgbvector = ListtoVector(rgblist);
		
		System.out.println(rgbvector.length);
		...
		/*嵌入后对文件信息的检查*/
        int[] barr1 = new  int[barr.length];//从嵌入信息图片中获取加密文件二进制流
		
		for(int i=0; i<barr.length; i++){
			barr1[i] = Parity(nrgbvector[i]);
		}
		...
		System.out.println("调用嵌入函数后，二进制不等的个数为："+c);
		//检测出c=0，则代表嵌入函数没有问题，即写图片或矩阵变化存在问题		
		...
		/*图片二维矩阵的生成*/
		ArrayList<int[]> nrgblist = VectortoList(nrgbvector,getImageRGB("F:/img1.jpg").length*getImageRGB("F:/img1.jpg")[0].length);
				
		int[] nrgbvector1= ListtoARGBVector(nrgblist);
		
		int[][] matrix = ARGBVectortoMatrix(nrgbvector1,getImageRGB("F:/img1.jpg")[0].length);
		//matrix是嵌入加密文件的二维数组
		...
		//二维数组的变换，转换成一位数组
		ArrayList<int[]> rgblist1 = getImageRGBList(matrix);
		
		int[] array = ListtoVector(rgblist1);
		...
		//嵌入文件后对比加密的二进制流
		int[] barr2 = new  int[barr.length];
		for(int i=0; i<barr.length; i++){
			barr2[i] = Parity(array[i]);
		}
		...
		System.out.println("矩阵变化后，二进制不等的个数为："+c1);
		//c1=0,则排除矩阵变化出现的错误，即错误在写图片
		
		createRGBImage(matrix, "F:/exp2/new.png", "png");
	    ...
	    //读取生成的图片，获取二进制
	    ArrayList<int[]> rgblist2 = getImageRGBList(getImageRGB("F:/new.png"));
		
		int[] array1 = ListtoVector(rgblist2);
		
		int[] barr3 = new  int[barr.length];
	
		for(int i=0; i<barr.length; i++){
			barr3[i] = Parity(array1[i]);
		}
	   ...
	   for(int i =0; i<barr.length; i++){
		...
		System.out.println("提取图片后，二进制不等的个数为："+c2);
		...
		//文件的写入
		byte[] bytearr = getByteArray(barr3);
		
		byte[] descryptarr = filekey.Decrypt(bytearr, "rgb4567890");
       
	   getFile(descryptarr,"F:","new.pdf");
	}
